...
public class EmailUtils {
	...
	public static void sendEmail(String senderEmailAccount, String senderEmailPasswd, 
			...
			String[] recverEmailAccount, String subject, String content) throws Exception{
    	Properties props = new Properties();                    
        props.setProperty("mail.transport.protocol", "smtp");   
        props.setProperty("mail.smtp.host", senderEmailSmtp);  
        props.setProperty("mail.smtp.auth", "true");           
        Session session = Session.getDefaultInstance(props);    
        session.setDebug(Boolean.parseBoolean(PropertiesUtil.getValue("mail.debug")));
	...
	public static void sendEmailByDefault(String subject, String content) throws Exception{
    	Properties props = new Properties();                    
        props.setProperty("mail.transport.protocol", "smtp");   
        props.setProperty("mail.smtp.host", SecretPropertiesUtil.getValue("mail.sender.smtp"));  
        props.setProperty("mail.smtp.auth", "true");           
        Session session = Session.getDefaultInstance(props);    
        session.setDebug(Boolean.parseBoolean(SecretPropertiesUtil.getValue("mail.debug")));
        MimeMessage message = new MimeMessage(session);
        message.setFrom(new InternetAddress(SecretPropertiesUtil.getValue("mail.sender.account"),
        		SecretPropertiesUtil.getValue("mail.sender.name")));
        for (String recv: SecretPropertiesUtil.getValue("mail.recver.account.default").split(",")){
        	message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(recv));
        }
        message.setSubject(subject);	
        message.setContent(content, "text/html;charset=UTF-8");
        ...
        message.saveChanges();	
        Transport transport = session.getTransport();
        transport.connect(SecretPropertiesUtil.getValue("mail.sender.account"), 
        		SecretPropertiesUtil.getValue("mail.sender.passwd"));
        transport.sendMessage(message, message.getAllRecipients());
        transport.close();
    }
	...
	public static void main(String[] args) throws Exception {
		...
		EmailUtils.sendEmailByDefault("工作上真的要小心", "积极，少犯错");
    }
}
